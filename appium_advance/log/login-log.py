...
time.sleep(5)
WebDriverWait(driver,1).until(lambda x:x.find_element_by_id('com.tencent.tws.gdevicemanager:id/choose_list'))  #元素等待设备类型选择界面
driver.find_element_by_xpath('//*[@class="android.widget.LinearLayout" and @index="0"]').click() #使用xpath方式点击渤海设备类型

logging.info('点击渤海P1')

WebDriverWait(driver,15).until(lambda x:x.find_element_by_id('com.pacewear.tws.phoneside.plugin.band2.pair:id/tv_start_pair'))  #元素等待开始配对界面
driver.find_element_by_id('com.pacewear.tws.phoneside.plugin.band2.pair:id/tv_start_pair').click() #点击开始配对

WebDriverWait(driver,10).until(lambda x:x.find_element_by_id('com.pacewear.tws.phoneside.plugin.band2.pair:id/listView'))  #元素等待搜索列表
logging.info('进入搜索列表')
time.sleep(5)
driver.find_element_by_xpath("//*[@resource-id='com.pacewear.tws.phoneside.plugin.band2.pair:id/textView'][@text='Pacewear 0487A']").click() #在列表中找到要配对的设备0487A

logging.info('开始配对0478A')
WebDriverWait(driver,15).until(lambda x:x.find_element_by_id('com.tencent.tws.gdevicemanager:id/tv_bottom_button'))  #元素等待开启通知界面
driver.find_element_by_id('com.tencent.tws.gdevicemanager:id/tv_bottom_button').click()
time.sleep(3)
driver.tap([(953,1102)])
logging.info('点击开关')
time.sleep(1)
